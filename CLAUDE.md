...
### Comments and activity (sub-resources, not in the board payload)

The only card data outside the full-board payload. Routes: `GET /api/boards/{id}/comments`, `POST /api/boards/{id}/cards/{cardId}/comments`, `DELETE /api/boards/{id}/comments/{commentId}`, `GET /api/boards/{id}/activity`. Tables `comments` and `activity` (created via `CREATE TABLE IF NOT EXISTS`, no migration). `comments.card_id` is a plain column, not an FK, because `replace_board` deletes and reinserts every card row; comments are keyed by `(board_id, card_id)` and only pruned for card ids a `PUT` actually removes. Author-only delete. The activity feed is append-only: `backend/app/activity.py` `diff_board_activity(old, new)` derives `card_created/edited/moved/deleted` and `column_renamed` entries from the before/after board snapshots inside `replace_board`; the comment routes write `comment_added`/`comment_deleted` directly. `AuthGate` loads both feeds per board and bumps a `feedVersion` counter after any board save, AI update, or comment mutation to refetch. Times render in UTC via `src/lib/datetime.ts` `formatTimestamp` (deterministic across environments).
